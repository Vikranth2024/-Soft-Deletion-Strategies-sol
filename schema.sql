-- =============================================================================
-- LedgerApp — Updated Schema (Soft Delete Refactor)
-- =============================================================================
...
--   NULL  → the record is active and visible to normal application queries
--   value → the record was soft-deleted at that timestamp; treat as gone for
...
--   retaining — their purpose is transient, not historical.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- Hard data deletion should NEVER occur here — user identity records
...
-- At scale, this index stays small as records accumulate — it never grows
...
-- Partial index for active accounts by user — the primary query pattern
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    -- at that time — but it MUST remain in the database permanently.
...
-- Partial index for active transactions by account — the single most-executed query
...
-- Composite partial index: account_id + active status — covers the exact query
