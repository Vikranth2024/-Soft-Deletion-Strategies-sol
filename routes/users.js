...
// GET /users ΓÇö return only active (non-deleted) users
...
// API responses ΓÇö a GDPR data exposure for users who exercised right to erasure.
...
// GET /users/:id ΓÇö return a single active user
...
// POST /users ΓÇö create a new user
// No change needed ΓÇö inserts are unaffected by soft-delete logic.
...
// PUT /users/:id ΓÇö update a user's name or email
...
// DELETE /users/:id ΓÇö soft-delete a user
...
// are never cascade-deleted ΓÇö they are independent audit records.
...
// GET /admin/deleted ΓÇö audit endpoint: return all soft-deleted users
// This route did not exist in the original codebase. Hard deletion made it impossible
// ΓÇö there were no deleted records to retrieve. Now that records are soft-deleted,
