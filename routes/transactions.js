...
// GET /transactions ΓÇö return all active (non-deleted) transactions
...
// dispute handlers have soft-deleted ΓÇö creating a confusing or inaccurate
// account statement.
...
// GET /transactions/account/:accountId ΓÇö return active transactions for an account
...
// transaction still appears on the user's statement ΓÇö a Truth in Lending Act
...
// GET /transactions/:id ΓÇö return a single active transaction
...
// POST /transactions ΓÇö record a new transaction against an account
...
// DELETE /transactions/:id ΓÇö soft-delete a transaction
...
//
// Under the Bank Secrecy Act (31 U.S.C. ┬º 5318), transaction records must be
...
// GET /admin/deleted ΓÇö audit endpoint: return all soft-deleted transactions
// This route is the centrepiece of the soft-delete refactor's compliance value.
// In the original codebase, this was impossible ΓÇö deleted transactions were gone.
...
router.get('/admin/deleted', async (req, res) => {
    try {
        ...
        if (account_id) {
            // Filtered by account ΓÇö the primary compliance query path
            query = `
