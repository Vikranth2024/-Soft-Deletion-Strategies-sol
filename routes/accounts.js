...
// GET /accounts ΓÇö return all active accounts
...
// see accounts they have already closed ΓÇö creating a confusing and potentially
...
// GET /accounts/user/:userId ΓÇö return active accounts for a specific user
...
// accounts the user can no longer transact on ΓÇö a Regulation E disclosure failure.
...
// GET /accounts/:id ΓÇö return a single active account
...
// POST /accounts ΓÇö open a new account for a user
...
// DELETE /accounts/:id ΓÇö soft-delete (close) an account
...
// its transaction history should disappear ΓÇö the two lifecycles are independent.
...
// GET /admin/deleted ΓÇö audit endpoint: return all soft-deleted (closed) accounts
// This route did not exist in the original codebase. Without soft delete, there
// was no such thing as a closed account record ΓÇö it was simply gone.
...
// which establishes exactly when the account was closed ΓÇö critical for dispute
